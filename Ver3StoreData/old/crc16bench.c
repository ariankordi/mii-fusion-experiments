...
// This table is assumed to be provided. It contains 256 entries.
...
/**
 * @brief Main function to benchmark the three CRC implementations.
 *
 * Generates a test string and benchmarks each of the three CRC functions over
 * multiple iterations. Prints out the computed CRC values and the elapsed time.
 */
int main(void) {
    // Define a test string. This could be replaced with any desired input.
    const char *testString = "The quick brown fox jumps over the lazy dog";
    size_t testLength = strlen(testString);

    // Variables to store CRC results.
    uint16_t crc_magic = 0;
    uint16_t crc_bitwise = 0;
    uint16_t crc_table = 0;

    clock_t start, end;
    double time_magic, time_bitwise, time_table;

    // Benchmark the "magic" implementation.
    start = clock();
    for (int i = 0; i < ITERATIONS; i++) {
        // For consistency, we pass an initial CRC of 0xFFFF.
        crc_magic = crc16_magic(0xFFFF, testString);
    }
    end = clock();
    time_magic = ((double)(end - start)) / CLOCKS_PER_SEC;

    // Benchmark the bitwise (non-table) implementation.
    start = clock();
    for (int i = 0; i < ITERATIONS; i++) {
        crc_bitwise = crc16_bitwise((const uint8_t *)testString, testLength);
    }
    end = clock();
    time_bitwise = ((double)(end - start)) / CLOCKS_PER_SEC;

    // Benchmark the table-driven implementation.
    start = clock();
    for (int i = 0; i < ITERATIONS; i++) {
        crc_table = crc16_table_driven((const uint8_t *)testString, testLength);
    }
    end = clock();
    time_table = ((double)(end - start)) / CLOCKS_PER_SEC;

    // Print out the computed CRCs and elapsed times.
    printf("Test String: \"%s\"\n", testString);
    printf("Iterations: %d\n\n", ITERATIONS);
    printf("Magic Implementation: 0x%04X, Time: %f seconds\n", crc_magic, time_magic);
    printf("Bitwise Implementation: 0x%04X, Time: %f seconds\n", crc_bitwise, time_bitwise);
    printf("Table-driven Implementation: 0x%04X, Time: %f seconds\n", crc_table, time_table);
